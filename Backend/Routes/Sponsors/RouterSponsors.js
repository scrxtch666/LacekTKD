...
router.post("/", upload.single("image"), (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) return res.status(500).json({ error: "Chyba při ukládání do databáze" });
...
router.put("/:id", upload.single("image"), (req, res) => {
  const { id } = req.params;
  const { sponsor_name, url } = req.body;
  if (!sponsor_name) return res.status(400).json({ error: "Chybí název sponzora" });

  if (req.file) {
    db.query("SELECT img_path FROM sponsors WHERE id = ?", [id], (err, results) => {
      if (err) return res.status(500).json({ error: "Chyba serveru" });
      if (!results.length) return res.status(404).json({ error: "Sponzor nenalezen" });
      ...
      db.query(
        ...
        (err2) => {
          if (err2) return res.status(500).json({ error: "Chyba při aktualizaci" });
    ...
    db.query(
      ...
      (err, result) => {
        if (err) return res.status(500).json({ error: "Chyba při aktualizaci" });
        if (!result.affectedRows) return res.status(404).json({ error: "Sponzor nenalezen" });
